...
7. 各項目を「## 記事の書き方（最重要）」に従って日本語化（`title_ja` / `summary_ja`（=「こんなことができます」オイカゼ業務 before→after・3行・合計100字以内） / `article`（この記事は何か・新聞小見出しスタイルで1段落150字以内・配列1要素） / `source_date` / `ideas`（ほかの使い方2つ） / `try_hint`）。`profile.json` の `interest_tags` で `tags` を付け、関連度の高い順に並べる。
...
```jsonc
{
  ...
  "headline_top": {
    "title_ja": "",
    "summary_ja": ["", "", ""],
    "article": ["①核の説明②動作③情報源を新聞小見出しスタイルで1段落にまとめる（150字以内・配列1要素）"],
...
### summary_ja（3行＝「こんなことができます」・折りたたみ時に見える最重要部分）
- **3行の合計を100字以内に収める**（1行あたり30〜35字が目安）。タイトルの次に見える最重要エリアなのでコンパクトに。
- カードを開く前から見える、**オイカゼの業務が before → after でどう変わるか**を3行で伝える。技術の転記でなく「使うと塾業務がこう変わります」が軸。
...
### article（記事・1段落＝折りたたみ時に「記事」ラベルで見える事実補完）
- **「この更新は何か」を客観的・専門用語なしで150字以内の1段落（配列1要素）で書く**。新聞の小見出し記事スタイル。
- `summary_ja`（活用イメージ）との役割分担：`summary_ja` = "使うとオイカゼがこう変わる"（主役）、`article` = "何の更新か"（事実・脇役）。
- **1文の中に①核の説明②動作・条件③情報源（いつ・誰が・どこで）を自然に混ぜて**読み切れる段落にする。箇条書き感を出さない。
- 専門用語はカッコで言い換える（例: スレッド→作業の流れ、commit→保存ポイント）。
- 例:
  ```json
  ["OpenAI Codex（app 26.616・6/18）で、あるPCで始めた作業セッションを別のマシンへ自動引き継ぎできるようになった。コピペ・環境の再構築は不要。Claude Codeには同等機能は未実装。OpenAI公式Codex changelogに記載。"]
  ```
...
- ❌ 悪い（転記・専門用語 / 活用イメージなし）:
  ...
  - article: なし
- ✅ 良い（オイカゼ業務 before→after ＋ article で事実補完）:
  - title:「AI が勝手に危険な操作をしないよう自動ガード（事故防止）」
  - summary_ja:（開発業務に当てはまる例）
    1.「塾アプリのデータを AI に触らせるとき、生徒情報の消去や請求データの上書きを自動で止めてくれる。」
    2.「AI に任せていても、取り消しできない操作には自動でブレーキがかかる。」
    3.「事故が減れば AI への委任を増やせる＝開発スピードは落とさず安心度が上がります。」
  - article:
    1.「AI がコードやデータを操作するとき、取り消しできない危険なコマンド（データ消去・強制上書きなど）を自動検知してブロックする機能。」
    2.「ブロックの対象は"元に戻せない"カテゴリに限定され、通常のファイル作成・編集・テスト実行は止まらない。」
    3.「Claude Code v2.1.183（6/19リリース）で追加。公式changelogが出どころ。」
