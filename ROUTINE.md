...
1. **🛠 実践・活用事例**（howto）… **「AIで実際に業務を片付けた・アプリを作った」実証事例。ツール不問**（ChatGPT/Gemini/Claude/各種SaaS）。「〜できます」「〜に使えます」という機能紹介は howto にしない——「やってみた・こうなった」の結果がある記事だけ。**目標2件**。japanese・note・practitioner_blogs を**最初に**探す。
...
3. **探索（事例ファーストの順で実施）**：
   ...
   **【第1段：日本語実践記事】**（japanese カテゴリ → 最初に探す）
   Zenn・note・Qiita・DevelopersIO・はてブ を直近7日のキーワード検索で探す。検索例：「Zenn 生成AI 業務効率化 やってみた」「note AI 非エンジニア 自動化 2026」「Qiita ChatGPT 業務 アプリ作った」。**Claude Code 限定にしない。非エンジニアが AI で業務を片付けた・アプリを作った体験記を最優先**。日本語ソースは鮮度が高く（翌日記事が多い）、塾長に一番刺さる。
...
- **事例ファースト：howto 1件以上必須。ツール不問の実証事例のみ（「〜できます」の機能紹介は howto にしない）。** 全件チェンジログ転記はNG → 第1〜3段に戻る。
