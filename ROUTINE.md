...
- **事例ファースト：howto 1件以上必須。** 全件チェンジログ転記だけはNG → 第1〜3段に戻る。
- **アンチ怠惰：releases.atom は最後。** 第1段→第2段→第3段を先に当たる。
- **同一ドメイン・同一リリースから2件以上採用しない。**
- **中の人ウォッチは sources.json の insider リストを起点に。** ブログ本文確認→🟩、X 本人発確認→🟨。又聞き・インプ稼ぎNG。
- sources.json と手順3の探索**以外**からは拾わない。
...
- **`ideas` は全項目に必須**（ほかの使い方2つ・同じ熱量で）。
- **ダッシュ（——／──）は使わない。** 読点や句点で自然につなぐ。
- 1号5記事目標・詰め込みすぎない。深掘りは `try_hint` のリンクに逃がす。
