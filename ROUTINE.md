...
5. 候補から既出 url（手順2）を `scripts/dedupe.js` の `dedupe` ロジックで除外する。
6. 各候補を評価：
...
7. 各項目を「## 記事の書き方（最重要）」に従って日本語化（`title_ja` / `summary_ja` 3行 / `try_hint`）。`profile.json` の `interest_tags` で `tags` を付け、関連度の高い順に並べる。
...
9. 新着が乏しい日は `quiet_day: true`、`headline_top` は省略し、evergreen な小ネタ（既存ツールの便利な使い方）を1件入れる。
