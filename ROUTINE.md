...
7. 各項目を「## 記事の書き方（最重要）」に従って日本語化（`title_ja` / `summary_ja`（=「こんなことができます」オイカゼ業務 before→after・3行） / `article`（この記事は何か・客観的に1〜2行） / `source_date` / `ideas`（ほかの使い方2つ） / `try_hint`）。`profile.json` の `interest_tags` で `tags` を付け、関連度の高い順に並べる。
...
### article（記事・1〜2行＝折りたたみ時に「記事」ラベルで見える事実補完）
- **「この更新は何か」を客観的・専門用語なしで1〜2行書く**（配列）。
- `summary_ja`（活用イメージ）との役割分担：`summary_ja` = "使うとオイカゼがこう変わる"（主役）、`article` = "何の更新か"（事実・脇役）。
- 例: `["OpenAI Codex で、自宅PCで始めたAIとの作業を、別のつながったマシンへそのまま移して続けられるようになった機能。"]`
...
- ✅ 良い（オイカゼ業務 before→after ＋ article で事実補完）:
  ...
  - article:「AI がコードやデータを操作するとき、取り消しできない危険なコマンド（データ消去・強制上書きなど）を自動検知してブロックする機能。Claude Code v2.1.183 で追加。」
