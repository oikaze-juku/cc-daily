...
7. 各項目を「## 記事の書き方（最重要）」に従って日本語化（`title_ja` / `summary_ja`（=「こんなことができます」オイカゼ業務 before→after・3行） / `article`（この記事は何か・客観的に2〜3行、①核の説明②動作詳細③情報源） / `source_date` / `ideas`（ほかの使い方2つ） / `try_hint`）。`profile.json` の `interest_tags` で `tags` を付け、関連度の高い順に並べる。
...
```jsonc
{
  ...
  "headline_top": {
    "title_ja": "",
    "summary_ja": ["", "", ""],
    "article": ["①何の機能か（核の説明）", "②具体的な動作・条件・使い方", "③情報源（いつ・誰が・どこで）"],
...
### article（記事・2〜3行＝折りたたみ時に「記事」ラベルで見える事実補完）
- **「この更新は何か」を客観的・専門用語なしで2〜3行（配列の要素を2〜3個）書く**。
- `summary_ja`（活用イメージ）との役割分担：`summary_ja` = "使うとオイカゼがこう変わる"（主役）、`article` = "何の更新か"（事実・脇役）。
- **3要素を各1文に分けて書く**：①何の機能か（核の説明）、②具体的な動作・条件・使い方、③情報源（いつ・誰が・どこで）。
- 専門用語はカッコで言い換える（例: スレッド→作業の流れ、commit→保存ポイント）。
- 例:
  ```json
  [
    "OpenAI Codex（app 26.616・6/18）で、あるPCで始めたAIとの作業を別のつながったマシンへそのまま移して続けられる機能が追加された。",
    "スクリプト不要で引き継ぎは自動。Claude Codeには同等機能は未実装だが、同方向への展開が各ツールで進んでいる。",
    "OpenAI公式Codex changelogに記載。"
  ]
  ```
