...
## 追記（2026-07-01）：auto-loop 要件確定

設計の運用形態を「週1ワークフロー」から **「毎晩の auto-loop（定期監視・自己改善）」** に確定。

| 項目 | 決定 |
|---|---|
| 形態 | auto-loop（終点なし・毎晩回して育てる）。ゴールではない |
| 時刻 | 毎晩 23:00 JST（号生成は翌朝6:00 の前） |
| 評価基準 | 段階導入：① ルーブリック自己採点（事例ファースト/鮮度/重複なし/profile関連度）→ ② ★が貯まれば ★率 を追加 |
| 行動範囲 | **枠つき自動修正**：interest_tags 並べ替え＝自動（revert可）／テーマ増減＝draft PR で提案・人間承認 |
| 安全網 | 全変更 git 履歴で revert 可能＋月1メタ監査（精度トレンド・多様性の痩せを点検） |
| 記録先 | `tuning_log.md`（人間が読む反省履歴）＋ `<slug>-state.jsonl`（生ログ） |
| 開始の前提 | ★収集(Firebase) を先に構築（本番操作＝人間ゲート）。それまで夜ループは登録しない |

**着手順**：①Firestore★収集（人間ゲート）→ ②夜23:00 auto-loop 登録 → ③数週観察 → ④メタ監査追加。
